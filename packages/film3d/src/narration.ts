...
export interface NarrationSpec {
  fps: number;
  engine: "espeak" | "elevenlabs";
  espeak: { voice: string; pitch: number; speed: number };
  /** Present when engine is "elevenlabs" (the API key travels via ELEVENLABS_API_KEY, never here). */
...
/** ElevenLabs premade "George" — a warm narration voice; override per call. */
 ...
 * muxer clips any tail past the film's end. Engine defaults to espeak (offline,
 * deterministic); "elevenlabs" swaps in a production voice with no other changes.
 */
export function narrationScript(
  doc: Film3DDoc,
  opts: { engine?: "espeak" | "elevenlabs"; voiceId?: string } = {},
): NarrationSpec | null {
  const lines = doc.beats
    .filter((b) => b.narration)
    .map((b) => ({ at: b.start + LEAD_IN, text: b.narration! }));
  if (lines.length === 0) return null;
  const engine = opts.engine ?? "espeak";
  return {
    fps: doc.fps,
    engine,
    espeak: NARRATOR,
    ...(engine === "elevenlabs" ? { elevenlabs: { voiceId: opts.voiceId ?? DEFAULT_ELEVENLABS_VOICE } } : {}),
    lines,
  };
}
