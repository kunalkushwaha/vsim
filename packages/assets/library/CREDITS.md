# Character asset library — credits & licenses

These sample rigged characters are bundled so scenes can load a ready-made character by id
(`loadCharacter("person")` / `loadCharacter("fox")`). They come from the
[glTF Sample Assets](https://github.com/KhronosGroup/glTF-Sample-Assets) repository and retain
their original licenses (vsim's own code is MIT).

## `cesium-man.glb` — "Person" (CesiumMan)
- © 2017, Cesium — [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/) (attribution required).
- The Cesium logo on the model is a **trademark** (our software renderer doesn't load textures, so
  the logo is not displayed; the file is included unmodified).

## `human.glb` / `man.glb` / `kid.glb` / `suited.glb` — MakeHuman humans
- Realistic rigged humans generated with **MakeHuman / MPFB 2** (makehumancommunity.org) — a woman,
  a man, a child, and a man in a suit, differing by MakeHuman's gender/age/build macros (baked into
  the mesh) but sharing the same rig + walk/run/idle/wave clips. MakeHuman output is **CC0** (public
  domain). Regenerate/customize via `scripts/blender/make-human.py`.
- `suited.glb` adds **real clothing geometry** — a casual suit + shoes from the same CC0 pack, fitted
  and skin-weighted to the rig (three skinned meshes), with the under-clothing body removed.
- `avatar.vrm` is a **VRM 1.0** avatar built CC0 from a MakeHuman human, with the `VRMC_vrm`
  humanoid/meta extension injected by `scripts/make-vrm.mjs` (the file declares CC0 in its VRM meta).
- `speaker.glb` adds a CC0 **"mouthOpen" morph target** (MakeHuman's jaw-drop target) for lip-sync.
- The **skin + clothing textures** come from MakeHuman's `makehuman_system_assets` pack
  ([CC0 1.0](https://creativecommons.org/publicdomain/zero/1.0/)) — baked to a single base-color
  map (`skin_type="GAMEENGINE"`) and downscaled to 1024². Pass any skin from that pack to the
  script to change it.
- The **walk** clip is real motion capture, retargeted onto the rig: **CMU Graphics Lab Motion
  Capture Database** (subject 07, trial 01 — walking), from [mocap.cs.cmu.edu](https://mocap.cs.cmu.edu/),
  free for all uses; BVH conversion via [una-dinosauria/cmu-mocap](https://github.com/una-dinosauria/cmu-mocap)
  (B. Hahne / cgspeed). Bundled at `scripts/blender/mocap/cmu_07_01_walk.bvh`. The run/idle/wave clips
  are vsim's own (hand-keyed). Retarget any BVH with `make-human.py … walk.bvh`.

## `dog.glb` — "Quadruped (Blender)"
- vsim's own asset, **MIT** — a rigged four-legged creature with walk/trot gaits, generated by
  `scripts/blender/make-quadruped.py` (Blender, headless). No third-party content.

## `knight.glb` / `barbarian.glb` / `mage.glb` / `rogue.glb` — KayKit Adventurers
- **KayKit Character Pack: Adventurers** by Kay Lousberg — [CC0 1.0](https://creativecommons.org/publicdomain/zero/1.0/)
  (free for personal & commercial use, no attribution required). Fetched from
  [KayKit-Game-Assets/KayKit-Character-Pack-Adventures-1.0](https://github.com/KayKit-Game-Assets/KayKit-Character-Pack-Adventures-1.0);
  files included unmodified. Each is fully textured and rigged with a large shared animation
  library (the manifest lists the curated subset the film3d cast uses).

## `deer.glb` / `bear.glb` / `rabbit.glb` — the animal cast (Blender)
- vsim's own assets, **MIT** — rigged, animated animals (walk/run gaits + a breathing idle)
  generated by `scripts/blender/make-animal.py` (Blender, headless) from a per-species table of
  bones, body parts, and gait parameters. No third-party content; add a species by adding a table.

## `figure.glb` — "Figure (Blender)"
- vsim's own asset, **MIT** — generated by `scripts/blender/make-character.py` (Blender, headless).
  No third-party content; regenerate or customize it with that script.

## `fox.glb` — "Fox"
- Model: © 2014, PixelMannen — [CC0 1.0](https://creativecommons.org/publicdomain/zero/1.0/) (public domain).
- Rigging & animation (Survey / Walk / Run): © 2014, @tomkranis — [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/).
- glTF conversion: © 2017, @AsoboStudio and @scurest — [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/).

Both files are unmodified. CC BY requires attribution — this file provides it.
